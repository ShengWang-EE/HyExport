...
The first question is whether high-quality offshore wind resources also define the lowest-cost and therefore most strategic hydrogen exporters. They do not. We assess Belgium, Denmark, France, Germany, Ireland, the Netherlands, Norway, Portugal, Spain, Sweden and the United Kingdom (UK), which combine large exclusive economic zones (EEZs), offshore infrastructure potential and navigable links to European demand centres. Wind velocity, air pressure and temperature data are obtained from ERA5 at 1 hour and 0.5° $\times$ 0.5° temporal and spatial resolutions \cite{ERA5}. Hourly capacity factors and wake-effect losses are simulated using a 15 MW reference wind turbine \cite{musial2019oregon}. Ireland has the highest modal wind speed among the studied EEZs, followed by the UK, because both are exposed to Atlantic wind resources (Fig. \ref{fig LCOH map}b).
...
\begin{figure}
    ...
    \textbf{a--c}, Green-hydrogen cost-supply curves for 2030, 2040 and 2050, respectively. Coloured lines show all studied countries; Denmark, Ireland, the Netherlands and the UK are drawn with thicker lines as the main comparisons discussed in the text. Coloured markers denote Ireland's and the UK's offshore wind capacity targets on their corresponding supply curves. Grey bands show the assumed future blue-hydrogen production-price range.}
...
Export value appears only after domestic absorption begins to bind. Because the offshore wind ambitions of Ireland and the UK exceed their projected domestic power demands, we estimate how much offshore production can be absorbed domestically before it becomes available for export. We use a whole-energy-system optimisation model that simulates hourly power and gas operation over a full year under uncertain offshore wind generation. Power and natural gas systems are coupled through gas-fired power plants and onshore/offshore electrolysers, allowing bidirectional energy conversion to expand system flexibility. Hydrogen blending and pure-hydrogen network cases are represented through different maximum hydrogen fractions in the gas network. Hourly unit commitment includes the All-Ireland generator fleet, interconnectors, reserve, inertia, must-run and system non-synchronous penetration constraints, and observed hourly demand profiles (Methods; Supplementary Information) \cite{OperationalPolicy}.

The domestic absorption result is strongly seasonal (Fig. \ref{fig unit commitment}). We assume a maximum 20\% hydrogen blending ratio in 2040, reflecting near-term blending demonstrations and policy movement, and a pure-hydrogen network in 2050 in line with the UK's hydrogen network roadmap \cite{BritainsHydrogenNetworkPlan}. Without export, the average summer wind-curtailment rate increases from 62.36\% in 2030 to 69.80\% in 2050 because high offshore generation coincides with low seasonal demand. In winter, the curtailment rate falls from 32.05\% in 2030 to 13.47\% in 2050 because higher hydrogen blending allows the gas system to absorb more offshore energy.
