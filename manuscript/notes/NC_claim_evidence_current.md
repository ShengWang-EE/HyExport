...
|---|---|---|---|---|
| Title, `sn-article.tex:105` | Cross-border offshore hydrogen trade and carbon mitigation in Europe. | Entire Results sequence; especially the trade/carbon figure, LCOH-only counterfactual and outside-option sensitivity. | Supported | Keep this object-led title logic unless the main story changes. |
| Abstract, `sn-article.tex:143` | The assessment links cost-supply curves, hourly power-gas operation, hydrogen/ammonia shipping, national demand and external import competition. | Methods describe all modules; current SI describes cost, domestic and trade modules separately. | Partial | Add novelty comparison table to SI and one sharper Introduction sentence. |
...
| Abstract, `sn-article.tex:143` | UK and Ireland each reach about 162 TWh yr^-1 net exports in 2050. | `NC_story_comparison.csv`: UK 162.141, Ireland 161.861 TWh yr^-1 under base integrated model; `NC_headline_numbers.csv` records the rounded claim. | Supported | Keep source-number table with the submission package. |
| Abstract, `sn-article.tex:143` | Modelled offshore hydrogen contributes about 175.5 Mt CO2 yr^-1 mitigation. | `NC_story_comparison.csv`: 175.499942 Mt CO2 yr^-1 in 2050 integrated model; `NC_headline_numbers.csv` separates total mitigation from European offshore supplier contribution. | Supported but method-sensitive | Clarify carbon mitigation attribution and displacement baseline in SI/Methods. |
| Abstract and Results, `sn-article.tex:323-325` | External imports near 3 EUR kg^-1 still suppress European offshore supply; around 3.3-3.5 EUR kg^-1 restores the UK/Ireland export pattern. | `NC_outside_option_sensitivity.csv`: 2050 European offshore supply is 98.2, 599.6 and 708.7 TWh at 3.0, 3.3 and 3.5 EUR kg^-1. | Supported | Add SI method subsection and full year-by-year table/figure. |
...
| Results 2, `sn-article.tex:265-280` | Domestic power-gas constraints create exportable surplus; Ireland reaches 170 TWh yr^-1 export potential. | `NC_ireland_curtailment_summary.csv`, `NC_domestic_export_potential.csv`, unit-commitment results and `EUwindConsump_new`; Fig. unit commitment and wind decomposition. | Supported for headline values; full hourly traces remain checkpoint-backed | Add domestic absorption sensitivity to SI and improve figure expression if another figure pass is requested. |
| Results 4, `sn-article.tex:307-318` | The integrated model and the lowest-LCOH allocation give different exporter geographies. | `NC_story_comparison.csv`; Fig. counterfactual. | Supported | SI should define baseline clearly and provide country table. |
| Results 3, `sn-article.tex:289-304` | Cross-border hydrogen and ammonia trade assigns carbon-mitigation value across suppliers and destinations. | Sankey figure and `solution.carbonReductionContributionMatrix`; `NC_story_comparison.csv`. | Supported but method-sensitive | Clarify H2/ammonia energy-equivalent units and carbon attribution. |
...
2. `NC_outside_option_sensitivity.csv` supports conditional robustness:
   ...
   - 2050 outside option 3.3 EUR kg^-1: European offshore supply 599.62 TWh; offshore becomes the majority source.
   - 2050 outside option 3.5 EUR kg^-1: European offshore supply 708.71 TWh; UK and Ireland return as large net exporters.
...
3. Carbon mitigation attribution is central and now has headline-number source tables, but still needs a concise SI explanation of the displacement baseline.
   - Fix: add the carbon attribution explanation to SI/Methods.
