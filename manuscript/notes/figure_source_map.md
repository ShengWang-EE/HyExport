...
|---|---|---|---|---|---|
| Fig. 1, `fig LCOH map` | `manuscript/figs/fig_LCOH_map_manu.pdf` | `scripts/plotFigures.m` lines 858-947, exports `figs/fig LCOH map manu.pdf` | `stop3.mat`, inherited from `stop1.mat`: `LCOH`, `latGrid_mesh`, `lonGrid_mesh`, `EUcountryList`, `nCountry`; also reruns `loadCountryClimate` for wind-speed densities | Offshore hydrogen cost differs spatially; Ireland and UK have rich wind resources but Denmark has the lowest LCOH due to combined wind and bathymetry conditions. | Keep the map concept, but consider moving the dense 11-country wind-density strips to SI or simplifying them. This figure should support resource-cost geography, not the full paper claim. |
...
| Fig. 3, `fig lcoh rank` | `manuscript/figs/fig_cost_table.pdf` | `scripts/plotFigures.m` lines 1274-1436, exports `figs/fig cost table.pdf` | `tables/tables.xlsx`, sheet `cost table`; values should be reconciled with `stop1.mat` output variables `averageLCOH`, `marginalLCOH`, `hyUnderBlue` | Average LCOH, marginal LCOH, and blue-hydrogen-competitive capacity rank countries differently. | Likely move to SI or compress into a small inset in Fig. 2. As a main figure it reads like a diagnostic table and duplicates the curve result. |
| Fig. 4, `fig unit commitment` | `manuscript/figs/fig_unit_commitment_main.pdf` | `scripts/plotFigures.m` lines 1041-1096, exports `figs/fig unit commitment main.pdf` | `stop3.mat`, inherited from `stop2.mat`: `interconnectorPowerNoExport`, `electricityGenerationNoExport`, `windCurtailmentNoExport`, `curtailmentRateNoExport`, `interconnectorPowerExport`, `electricityGenerationExport`, `windCurtailmentExport`, `curtailmentRateExport` | Export access sharply reduces offshore wind curtailment in the Ireland power-gas case, especially in high-offshore-wind summer scenarios. | Redesign. The current 12 small time-series panels are too busy for a central NC figure. Main figure should show the mechanism and headline reductions; the full 12-panel dispatch result can go to SI. |
| Fig. 5, `fig wind decomposition` | `manuscript/figs/fig_wind_decomposition.pdf` | `scripts/plotFigures.m` lines 1223-1272, exports `figs/fig wind decomposition.pdf` | `stop3.mat`, inherited from `stop2.mat`: `EUwindConsump_new` | Domestic power-gas absorption limits create exportable surplus; the UK and Ireland become high-surplus countries by 2050. | Redesign as stacked bars or slope charts by country/year. The circular bar plot is hard to read and makes exact comparison difficult. |
...
3. Redesign `fig_unit_commitment_main.pdf` into a mechanism summary and move the detailed 12-panel dispatch figure to SI.
4. Redesign `fig_wind_decomposition.pdf` as bars/slope charts.
