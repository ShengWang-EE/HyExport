...
|---|---|---|---|---|---|
| Fig. 1, `fig LCOH map` | `manuscript/figs/fig_LCOH_map_manu.pdf` | `scripts/figures/plotLCOHMapAndWindDensities.m`; `scripts/plotFigures.m` calls the same dedicated reflow function; side-layout source is `figs/fig LCOH map manu side source.pdf` | `stop3.mat`, inherited from `stop1.mat`: `LCOH`, `latGrid_mesh`, `lonGrid_mesh`, `EUcountryList`, `nCountry`; wind-density panels are preserved from the prior side-layout source figure | Offshore hydrogen cost differs spatially; Ireland and UK have rich wind resources but Denmark has the lowest LCOH due to combined wind and bathymetry conditions. | Main map stays large; the 11 country wind-density panels are retained below the map. Image-based export avoids excessive vector complexity from EEZ boundaries and gridded surfaces. |
...
| SI cost-rank figure, `fig supplementary cost rank` | `manuscript/supplementary/J14___Supplementary_Information_v0_2/figs/fig_cost_table.pdf` | `scripts/plotFigures.m` lines 1274-1436, exports `figs/fig cost table.pdf` | `tables/tables.xlsx`, sheet `cost table`; values should be reconciled with `stop1.mat` output variables `averageLCOH`, `marginalLCOH`, `hyUnderBlue` | Average LCOH, marginal LCOH, and blue-hydrogen-competitive capacity rank countries differently. | Moved from main text to SI so the main Results stay claim-led. |
| Fig. 4, `fig unit commitment` | `manuscript/figs/fig_unit_commitment_main.pdf` | `scripts/figures/plotDomesticAbsorptionAndExportPotential.m`; legacy 12-panel version remains in `scripts/plotFigures.m` | `stop3.mat`: `curtailmentRateNoExport`, `curtailmentRateExport`, `windCurtailmentNoExport`, `windCurtailmentExport`, `electricityGenerationNoExport`, `electricityGenerationExport` | Export access sharply reduces offshore wind curtailment in the Ireland power-gas case and converts otherwise curtailed 2050 summer wind into usable energy. | Redesigned as a two-panel mechanism summary for the main manuscript. |
| Fig. 5, `fig wind decomposition` | `manuscript/figs/fig_wind_decomposition.pdf` | `scripts/figures/plotDomesticAbsorptionAndExportPotential.m`; legacy circular version remains in `scripts/plotFigures.m` | `stop3.mat`: `EUwindConsump_new` | Domestic power-gas absorption limits create exportable surplus; the UK and Ireland become high-surplus countries by 2050. | Redesigned as stacked country-year bars plus key-country export-potential trajectories. |
...
4. Use the redesigned `fig_wind_decomposition.pdf` as the main export-potential figure; keep country-level raw tables in SI/source data.
