...
| Fig. 2, `fig LCOH curve` | `manuscript/figs/fig_LCOH_curves_manu.pdf` | `scripts/figures/plotMainCostFigures.m`, called from `main.m`; exports `figs/fig LCOH curves manu.pdf` | `stop3.mat`, inherited from `stop1.mat`: `LCOHcurve2030`, `LCOHcurve2040`, `LCOHcurve2050` | Cost-supply curves show that the lowest-cost country is not necessarily the country with the largest strategic export value. | Keep all 11 country curves in the main figure, but highlight Denmark, Ireland, Netherlands and the UK while greying secondary countries to reduce line clutter. |
