...
Status note: this is a historical working audit from before the Nature Communications rewrite. Use `NC_claim_evidence_current.md` and `NC_headline_numbers.md` for the current source of truth.
...
| Results, current domestic-operation paragraph | Export access reduces Ireland's 2050 summer curtailment to 10.46%. | Fig. 4/unit commitment; `results/tables/NC_ireland_curtailment_summary.csv`; `curtailmentRateExport`. | Supported for headline value | Full hourly dispatch traces remain checkpoint-backed. |
| Results, current export-potential paragraph | Ireland becomes Europe's second-largest available export-potential country by 2050 with about 170 TWh yr^-1. | Fig. 5/wind decomposition; `results/tables/NC_domestic_export_potential.csv`; `EUwindConsump_new`. | Supported for headline value | Keep "available export potential" separate from optimized net export. |
