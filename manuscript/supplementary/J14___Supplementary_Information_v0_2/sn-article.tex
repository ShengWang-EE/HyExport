...
Section 2 positions the study relative to representative offshore hydrogen cost, infrastructure, demand and global trade studies.
Section 3 introduces the wind resources in Ireland's marine areas, as well as physical models of offshore wind turbine arrays considering the impacts of the wake effect.
Section 4 develops frameworks to evaluate the levelized production cost for multiple offshore resource utilisation schemes, including electricity, liquefied hydrogen, and ammonia.
Using these data as inputs, Section 5 proposes a multi-energy systems optimisation framework for power systems and gas systems with hydrogen integration, and investigates the domestic integration potential of renewable electricity and green hydrogen from offshore wind.
Based on the remaining capacity of hydrogen from domestic integration and cost-supply curves, Section 6 develops an international trading model to project the cross-border hydrogen trading landscape. Carbon mitigation is also analysed to highlight Ireland's contribution to reshaping Europe's net-zero transitions.
...
\caption{Feature comparison between representative prior study types and the present integrated assessment. A check mark indicates that the feature is explicitly represented and coupled to the study workflow. A dash indicates that the feature is outside the main scope of that study type. Partial means that the feature is discussed or parameterised, but not jointly optimised with all other columns.}
...
The comparison is intended to define the integration boundary of the present assessment rather than to diminish the value of prior studies. The cited offshore hydrogen studies provide production-cost, route or uncertainty benchmarks, while infrastructure and trade outlooks provide demand, corridor and global import context. The present study links these strands into one European optimisation workflow so that exporter rankings, external-import dependence and supplier-specific carbon-mitigation contributions can be evaluated together.
...
The main manuscript uses the hourly Ireland power-gas optimisation to estimate how much offshore production can be absorbed domestically, then scales the resulting domestic absorption by country-level electricity and gas demand before applying country export limits in the European trade model. This extrapolation is designed as a European screening step: it transfers the system-level relationship between offshore generation, electricity demand and gas-network hydrogen absorption, but it does not claim to reproduce the detailed unit-commitment operation of every country. To test whether this bridge drives the strategic-exporter conclusion, we multiply the domestic absorption estimate by 0.75 and 1.25 and resolve the integrated trade model. A lower factor means less domestic absorption and therefore more available export potential; a higher factor means more domestic absorption and therefore less available export potential. The perturbation is applied to domestic absorption directly because the main uncertainty being tested is the export limit passed from the domestic model into the international trade model.

\begin{table}[h]
\centering
\tiny
\setlength{\tabcolsep}{1pt}
\caption{Sensitivity of integrated trade results to domestic absorption scaling. Low and high cases multiply the base domestic absorption estimate by 0.75 and 1.25, respectively. Export potential (Export pot.), European offshore supply (Offshore), external imports (Import), Ireland net exports (IE net) and UK net exports (UK net) are reported in TWh yr$^{-1}$.}
...
The sensitivity changes the amount of European offshore supply that can compete with external imports, but it does not restore the lowest-LCOH exporter geography. In 2040, European offshore supply ranges from 380 to 610 TWh yr$^{-1}$ and external imports range from 4 to 229 TWh yr$^{-1}$ across the tested domestic-absorption factors. In 2050, European offshore supply ranges from 571 to 830 TWh yr$^{-1}$ and external imports range from 0 to 254 TWh yr$^{-1}$. These changes show that domestic absorption is a material system constraint, not a cosmetic adjustment. However, the strategic interpretation remains stable: the integrated model does not revert to the LCOH-only allocation, and the UK and Ireland remain the key high-surplus island systems in the 2050 cases. Lower domestic absorption strengthens the UK as the largest net exporter, while higher domestic absorption makes Ireland the largest net exporter. Under the base case the two countries are nearly tied, at about 162 TWh yr$^{-1}$ each. The exact UK/Ireland ordering is therefore sensitive to domestic absorption, but the conclusion that integrated domestic-absorption and trade constraints reshape the exporter ranking is preserved.
...
The country-wide average unit fuel cost of shipping hydrogen is shown in Fig. \ref{fig ship cost}. The cost depends on the shipping routes' lengths, which can be obtained from SeaRates \cite{SEAPATES}. Considering the around 60 \texteuro{} MWh$^{-1}$ marginal price for hydrogen, the transportation fuel cost accounts for around 0.08\% - 2.12\%. From Fig. \ref{fig ship cost}. a to Fig. \ref{fig ship cost}.c, the fuel cost will generally decline due to the reduced cost of hydrogen. Among different shipping routes, the ones connected to Portugal, such as Sweden and Norway, have relatively higher costs due to the distant locations.
